    ...
    <>
      ...
      <div className="fixed right-0 top-0 h-full z-50 w-full max-w-md bg-[var(--card)] border-l border-[var(--border)] shadow-2xl flex flex-col animate-fade-up">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-[var(--border)]">
          <div>
            <h2 className="font-display text-2xl tracking-widest accent-text">WATCHLIST</h2>
          ...
          <div className="flex items-center gap-2">
            <a
              href="/baseball/watchlist"
              onClick={onClose}
              className="text-xs accent-text hover:underline border border-[var(--accent)] border-opacity-30 accent-dim-bg px-2.5 py-1.5 rounded-lg"
            >
              Full page →
            </a>
            <button
        ...
        <div className="flex-1 overflow-y-auto">
          {flaggedPlayers.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center px-6 space-y-3">
              ...
              <h3 className="text-lg font-semibold text-[var(--text)]">No Players Flagged</h3>
            ...
            <div className="divide-y divide-[var(--border)]">
              {flaggedPlayers.map((p: RankedPlayer) => (
                <div key={p.id} className="flex items-start gap-3 px-5 py-3 hover:bg-[var(--navy-2)] transition-colors">
                  ...
                  {/* Unflag */}
                  <button
                    onClick={() => unflagPlayer(p.id)}
                    className="shrink-0 text-[var(--gold)] hover:text-[var(--text-dim)] text-lg leading-none"
        ...
        {flaggedPlayers.length > 0 && (
          <div className="px-5 py-3 border-t border-[var(--border)]">
            <a
              href="/baseball/watchlist"
              onClick={onClose}
              className="block w-full text-center py-2 rounded-lg accent-bg text-[var(--navy)] font-semibold text-sm hover:opacity-90"
            >
              View Full Watchlist
            </a>
          </div>
