...
interface TeamBankProps {
  /** When true, renders only the bank list (no internal Draft sub-tab). Used when embedded in DraftMode. */
  embeddedMode?: boolean;
  /** Callback to navigate to My Draft tab (used when embeddedMode=true) */
  onGoToDraft?: () => void;
}

export default function TeamBank({ embeddedMode = false, onGoToDraft }: TeamBankProps = {}) {
  const {
  ...
  const handleConfirmSave = () => {
    ...
    setSaveName('');
    if (!embeddedMode) setActiveTab('bank');
  };

  // Compute category coverage for a set of picks
  ...
  // ── Bank list renderer (shared between embedded and standalone) ────────────
  const bankList = (
      ...
      {bankedTeams.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 text-center space-y-3">
          ...
          <p className="text-[var(--text-dim)] max-w-sm">
            Build a team in My Draft, then save it here for comparison.
          </p>
          {(embeddedMode ? onGoToDraft : () => setActiveTab('draft')) && (
            <button
              onClick={embeddedMode ? onGoToDraft : () => setActiveTab('draft')}
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
          {bankedTeams.map((team: BankedTeam) => {
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-1 text-xs text-[var(--text-dim)]">
                      ...
                      {teamReserves.length > 0 && <span>+{teamReserves.length} reserves</span>}
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    ...
                    {teamReserves.length > 0 && (
                      <button
                        onClick={() => {
                          if (isSwapping) { setSwapTeamId(null); setSelectedSwapPick(null); }
                          else { setSwapTeamId(team.id); setSelectedSwapPick(null); setExpandedId(team.id); }
                        }}
                    ...
                    {team.picks.length === 0 ? (
                      ...
                      <>
                        {fillRosterSlots(team.picks, leagueSettings.rosterSlots as Record<string, number>, positionOverrides, noPD).map(({ slot, pick }, i) => (
                          <div
                            key={slot + i}
                            onClick={() => { if (isSwapping && pick) handleSwapClick(team.id, 'starter', pick.player.id); }}
                            className={`flex items-center gap-2 px-2 py-1 rounded text-xs transition-colors ${pick ? 'bg-[var(--navy-2)]' : 'border border-dashed border-[var(--border)]'} ${isSwapping && pick ? 'cursor-pointer hover:border hover:border-yellow-500/50' : ''} ${isSwapping && selectedSwapPick?.type === 'starter' && pick?.player.id === selectedSwapPick.playerId ? 'border border-yellow-500 bg-yellow-900/20' : ''}`}
                          >
                          ...
                          <>
                            ...
                            {teamReserves.map((r, i) => (
                              <div
                                ...
                                className={`flex items-center gap-2 px-2 py-1 rounded text-xs bg-[var(--navy-2)] opacity-80 transition-colors ${isSwapping ? 'cursor-pointer hover:border hover:border-yellow-500/50' : ''} ${isSwapping && selectedSwapPick?.type === 'reserve' && r.player.id === selectedSwapPick.playerId ? 'border border-yellow-500 bg-yellow-900/20 opacity-100' : ''}`}
                              >
  ...
  // ── Embedded mode: just the bank list + save button ───────────────────────
  if (embeddedMode) {
    return (
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <p className="text-sm text-[var(--text-dim)]">
            {bankedTeams.length} saved team{bankedTeams.length !== 1 ? 's' : ''}
            {myTeam.length > 0 ? ` · ${myTeam.length} players in current draft` : ''}
          </p>
          {myTeam.length > 0 && (
            <button
  ...
  return (
    <div className="space-y-6">
      ...
      {activeTab === 'draft' && <MyDraft onSaveToBank={openSaveModal} />}
